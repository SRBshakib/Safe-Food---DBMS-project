...
$dbname = "traceroots2.0"; // Replace with your actual database name
...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if ($action == 'accept') {
        ...
        if ($result->num_rows > 0) {
            ...
            $insertSql = "INSERT INTO $table (first_name, last_name, email, username, password, gender, affiliation_date)
                          VALUES ('".$user['first_name']."', '".$user['last_name']."', '".$user['email']."', '".$user['username']."', '".$user['password']."', '".$user['gender']."', NOW())";

            if ($conn->query($insertSql) === TRUE) {
                // Update status of the user to "accepted" in the pending_users_t table and set the affiliation date
                $updateSql = "UPDATE pending_users_t SET status = 'accepted', affiliation_date = NOW() WHERE id = '$userId'";
    ...
    if ($action == 'reject') {
        ...
        $updateSql = "UPDATE pending_users_t SET status = 'rejected' WHERE id = '$userId'";
        $conn->query($updateSql);
    }
}
